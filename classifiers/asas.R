...
data1 <- subset(data1,
                sources.classification %in% c("Beta Lyrae","Algol (Beta Persei)","Beta Cephei"))
...
## make sure df1's features are numeric
lapply(df1,class)
## two names were bad
df1$p2p_scatter_2praw <- as.numeric(as.character(df1$p2p_scatter_2praw))
df1$freq1_harmonics_freq_0 <- as.numeric(as.character(df1$freq1_harmonics_freq_0))
...
plot(df2$eclipsing_prob,
     rnorm(nrow(df2),sd=.1) + as.numeric(df2$classification),
     col=df2$classification)
...
period2 <- 2/(df2[df2$filename == filenames[ii],
                  "freq1_harmonics_freq_0"])
period
period2
...
pdf(paste("asas_candidates/",sub(".xml",".pdf",filenames[ii])))
DrawEclipsing2(tfe,period,period2,main=sub(".xml","",filenames[ii]))
dev.off()
